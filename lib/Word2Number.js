    ...
    findNumbersIndexes(words) {
        ...
        for (let i = 0; i < words.length; i++) {
            ...
            if (word.match(digitWordsRegex)) {
                if (lastNumberType == Enum.NUMBER_TYPES.NUMBER && this.checkNumberDigit(numberLength, numberType)) { // TODO: Numerik tipleri kontrol et!
                    if (!(item.startIndex >= 0)) {
                        item.startIndex = i;
                    } else {
                        item.endIndex = i;
                    }
                } else if (lastNumberType != -1 && this.isNumberDigitBigger(lastNumberType, numberType)) { // Şu anki sayının basamağı, bir önceki sayının basamağından büyük. Demek ki yeni sayı başlamış, ayırmalısın.
                    if (!(item.endIndex >= 0)) {
