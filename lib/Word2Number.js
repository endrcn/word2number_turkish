...
class Word2Number {
    ...
    converToNumber(numberWords) {
        ...
        for (let i = 0; i < numberWords.length; i++) {
            ...
            if (numberType == Enum.NUMBER_TYPES.DIGIT || numberType == Enum.NUMBER_TYPES.TEN) {
    ...
    __getDigitWordsRegex() {
        return new RegExp("(" + this.zeroWords.join("|") + "|" + this.magnitudeWords.join("|") + "|" + this.digitWords.join("|") + "|" + this.hundredWords.join("|") + "|" + this.tenWords.join("|") + ")", "ig");
    }
    ...
    findNumbersIndexes(words) {
        ...
        for (let i = 0; i < words.length; i++) {
            ...
            if (word.match(digitWordsRegex)) {
                ...
                if (lastNumberType != -1 && this.isNumberDigitBigger(lastNumberType, numberType)) { // Şu anki sayının basamağı, bir önceki sayının basamağından büyük. Demek ki yeni sayı başlamış, ayırmalısın.
                    if (!(item.endIndex >= 0)) {
